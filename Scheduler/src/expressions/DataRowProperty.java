...
public class DataRowProperty<T extends DataRow> extends SimpleObjectProperty<T> {
    ...
    public DataRowProperty(T initialValue) {
        super(initialValue);
        newRow = new PredicateBinding((T value) -> value.getRowState() == DataRow.ROWSTATE_NEW);
        deleted = new PredicateBinding((T value) -> value.getRowState() == DataRow.ROWSTATE_DELETED);
        modified = new PredicateBinding((T value) -> value.getRowState() == DataRow.ROWSTATE_MODIFIED);
    ...
    public DataRowProperty(Object bean, String name) {
        super(bean, name);
        newRow = new PredicateBinding((T value) -> value.getRowState() == DataRow.ROWSTATE_NEW);
        deleted = new PredicateBinding((T value) -> value.getRowState() == DataRow.ROWSTATE_DELETED);
        modified = new PredicateBinding((T value) -> value.getRowState() == DataRow.ROWSTATE_MODIFIED);
    ...
    public DataRowProperty(Object bean, String name, T initialValue) {
    ...
    protected class PredicateBinding extends BooleanBinding {
        ...
        PredicateBinding(Predicate<T> predicate) {
            super.bind(DataRowProperty.this);
            this.predicate = predicate;
        }
        @Override
        protected boolean computeValue() {
            T value = DataRowProperty.this.get();
            return value != null && predicate.test(value);
        }

        @Override
        public ObservableList<?> getDependencies() {
            return FXCollections.singletonObservableList(DataRowProperty.this);
        }

        @Override
        public void dispose() {
            super.unbind(DataRowProperty.this);
            super.dispose();
        }
    }
}
